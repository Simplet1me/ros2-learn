...
public:
  Talker():Node("Talker_c"){
    RCLCPP_INFO(this->get_logger(),"丙方(c)创建");
    //创建发布器和计时器，每0.5s发布一次报价确保甲收到
    publisher_ = this->create_publisher<Booksupply>("book_supplyc",10);
