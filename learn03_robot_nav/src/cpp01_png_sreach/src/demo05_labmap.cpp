...
public:
  MapPublisher() : Node("tf_broadcaster_node_cpp") {
    ...
    travel_map_publisher_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("travel_map",10);
    pcpublisher_ =
    ...
    std::cout<<"width_min = "<<width_min<<"height_min = "<<height_min<<std::endl;
...
private:
  std::string path =
      "/home/simpletime/Project/learn03_robot_nav/cloud_filtered.pcd";
     ...
     while (!openlist.empty()) {
      ...
      if (current->x == end_point->x && current->y == end_point->y) {
        ...
        for (const auto &p : path) {
          ...
          travel_map.data[p.second * width + p.first] = 1;
        }
