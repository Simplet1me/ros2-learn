...
public:
  PointCouldPublisher():Node("pc_broadcaster_node_cpp1"){
    ...
    pcpublisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("pt_trun",10);
  }
...
private:
  ...
  std::string path = "/home/simpletime/Project/learn02_pcl_spin/source/top3.pcd";


  void do_cb(const sensor_msgs::msg::PointCloud2 & get_cloud_msg){
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_sml(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_big(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    //每个网格内会有一个或多个点
    std::vector<std::vector<std::vector<std::vector<pcl::PointXYZ>>>> voxel_grid(grid_size_x,std::vector<std::vector<std::vector<pcl::PointXYZ>>>
                                                                                (grid_size_y,std::vector<std::vector<pcl::PointXYZ>>
                                                                                (grid_size_z)));


    //将点存入网格(小点云)
    for(auto& point : cloud_sml->points){
      //点的X坐标-网格最小边界 / 网格大小 = 当前网格坐标 （即X轴上第几个网格） = 网格坐标
